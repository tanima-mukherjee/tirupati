...
if(isset($_REQUEST['order_id']) && isset($_REQUEST['xec']) && isset($_REQUEST['code'])){
...
if($jsonarr['success'] != 1)
{
	$status_flag = 3;
		...
		$sql0 = "SELECT * FROM room_booking WHERE id=".$_REQUEST['order_id']." AND email='".$emailAddr."' AND order_id='".$_REQUEST['code']."'";
		...
		$sql = "UPDATE room_booking SET order_status='4',payment_id ='".$payment_id."'  WHERE id=".$_REQUEST['order_id'];
		mysqli_query($con,$sql);
		
		$sql2 = "SELECT full_name,email,phone,room_charge,order_id FROM room_booking where id = '".$_REQUEST['order_id']."'";
		...
		$smsMessage = "Thanks for your booking accommodation at Tirupati. order no. ".$arr['order_id']." amounting to ".$arr['room_charge']."/- was successful.";
	...
	*/
	...
	<p>Your accommodation booking for Order No. ".$arr['order_id']." amounting to Rs. ".$arr['room_charge']."/- was successful.</p>
...
?>
	...
	<div class="thanks-wrap">
		<div class="thanks-container">
			<img src="images/thankyou-icon.png" alt="">
			<h1>Thank You</h1>
			<p>Your submission is received and didn't complete payment. We will contact you soon.</p>
		</div>
	</div>
