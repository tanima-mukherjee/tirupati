...
?>
<?php $sql = "SELECT * FROM `site_settings` where title = 'ticket' "; 
							$res = mysqli_query($con,$sql);				
			...
			<div class="step-booking-box">
				...
				<div class="step-booking-top-middle">
					<div class="booking-datepicker">
						<div id="calendarTemplate" class="calenderwrap">
							
						</div>
						<input type="hidden" id="ticket" name="ticket" value="<?php echo $data['value'] ?>" readonly>
					...
					<div class="row">
						...
						<div class="col-sm-4">
							<div class="form-group">
							<label class="label-field">Number of Extra Ladoos: </label>
							<select class="form-control" name="ladoos" id="ladoos">	

								<?php if ($devotees != '') { 
											$number_of_ladoos = $devotees * 2;
								 } else { 
											$number_of_ladoos = 20;
								} ?>
								...
								for($i=0;$i<=$number_of_ladoos;$i++)

								{	?>									
									 <option <?php if($ladoos == $i){ echo 'selected="selected"'; } ?>><?php echo $i ?></option>
								<?php }
								?>
